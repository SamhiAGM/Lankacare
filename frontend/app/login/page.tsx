...
import { Activity, ShieldCheck, Lock, Mail, ArrowRight, CheckCircle2 } from 'lucide-react';
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl shadow-2xl p-6 sm:p-8 backdrop-blur-md space-y-6">
          ...
          <div className="p-4 rounded-xl bg-slate-800/80 border border-slate-700/80 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="teal">Multi-System Architecture</Badge>
            </div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Select any role to experience its dedicated, separate system portal with enforced data scopes:
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
              {[
                { role: UserRole.CITIZEN, label: 'Citizen Portal', desc: '/citizen • Public Health' },
                { role: UserRole.DOCTOR, label: 'Doctor Portal', desc: '/doctor • Clinical & Roster' },
                { role: UserRole.NURSE, label: 'Nurse / Staff', desc: '/staff • Ward Care' },
                { role: UserRole.PHARMACIST, label: 'Pharmacist', desc: '/pharmacist • Stock & Expiry' },
                { role: UserRole.HOSPITAL_ADMIN, label: 'Hospital Admin', desc: '/hospital-admin • Kinniya' },
                { role: UserRole.DISTRICT_ADMIN, label: 'District Officer', desc: '/district-officer • Trinco' },
                { role: UserRole.PROVINCIAL_ADMIN, label: 'Provincial Officer', desc: '/provincial-officer • Eastern' },
                { role: UserRole.MINISTRY_OFFICER, label: 'Ministry Officer', desc: '/ministry • National View' },
                { role: UserRole.MINISTRY_ADMIN, label: 'Ministry Admin', desc: '/ministry-admin • Governance' },
                { role: UserRole.DATA_ADMIN, label: 'Data Admin', desc: '/data-admin • Data Pipeline' },
                { role: UserRole.SUPER_ADMIN, label: 'System Admin', desc: '/system-admin • Infrastructure' },
              ].map((item) => (
                <button
                  key={item.role}
                  type="button"
          ...
          <form onSubmit={handleStandardLogin} className="space-y-4">
            ...
            <Button
              ...
              className="w-full"
            >
              Sign In to Health Portal <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          </form>
